...
cowplot::plot_grid(p_km[[1]],p_lrt)
```
...
cowplot::plot_grid(p_km[[1]],p_rmst)
```

Next we might try a statistic based on the difference in milestone survival probabilities at 18 months (via the Kaplan-Meier estimates), which is almost like giving everyone with an event prior to 18 months a score of 1 and everyone still alive at 18 months a score of -1 (there is a small correction for censoring).
...
cowplot::plot_grid(p_km[[1]],p_surv)
```

## Weighted log-rank tests

Weighted log-rank tests can also be compared in this framework. For example, a [modestly-weighted log-rank test](https://onlinelibrary.wiley.com/doi/full/10.1002/sim.8186) ($t^*= 9$).
...
cowplot::plot_grid(p_km[[1]],p_mwlrt)
```
...
By looking carefully at these graphs we can get a sense about whether a particular test is putting more emphasis on early parts of follow-up or late parts of follow-up. Here, for example, we see that for RMST the scores have a wide range at early follow-up times but do not change at later follow-up times, and vice-versa for the milestone and modestly-weighted tests. The log-rank test has more gradually changing scores across the whole follow-up period.
...
We use the concept of pseudo-values following [Andersen et al. (2017)](https://onlinelibrary.wiley.com/doi/full/10.1002/sim.7297).
