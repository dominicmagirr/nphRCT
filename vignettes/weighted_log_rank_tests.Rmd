...
This function requires the user to input recruitment times of the patients through
parameters `rec_c` and `rec_e`. These can be simulated using the function `sim_rec_times` which offers
recruitment time simulation via either a piecewise constant model or a power model.

The function `sim_events_delay` assumes that the survival times on the control arm follow an exponential distribution, and the survival times on the experimental arm follow a two-piece exponential distribution. Given rate parameter $\lambda$, the exponential distribution has the form:

\[
f(t)=\lambda \exp(-\lambda t)
\]

These rate parameters are set using the argument `lambda_c` for the control arm, `lambda_e_1` for the first part of the experimental arm, and `lambda_e_2` for the second part of the experimental arm.

The `sim_events_delay` function requires that the user sets a parameter `max_cal_t`, this is the calendar time at which all observations are censored.
...
The values of the weights in the log-rank test can be calculated using the function `find_weights` with argument `wlr="lr"`. In the case of the standard log-rank test, the weights are clearly very simple.
...
Again the weights can be calculated using the `find_weights` function and setting `wlr="fh"`, along with arguments `rho` and `gamma`.
