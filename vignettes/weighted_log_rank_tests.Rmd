...
The simulation method assumes that the survival times on the control arm follow an exponential distribution, and the survival times on the experimental arm follow a two-piece exponential distribution. Given rate parameter $\lambda$, the exponential distribution has the form:
...
The simulation method also models recruitment onto the trial, so that individuals are entered into the trial at some point after calendar time 0. Specifically, the power model is used
...
The simulation method in the `sim_events_delay` function also requires that the user sets a parameter `max_cal_t`, the calendar time at which all observations are censored.
...
## Performing a weighted log-rank test
...
### Test statistic and Z-statistic
